...
//     if (!existing) return;
//
//     //  Grab the primary subscription item
...
//
//     if (!plan) {
//       this.logger.warn(`Could not map Stripe price ${priceId} to a known plan`);
//       return;
//     }
//
//     // Safely extract the timestamp from the item (New API) or the root (Old API)
//     // We cast to `any` to prevent TypeScript errors if SDK is updated.
