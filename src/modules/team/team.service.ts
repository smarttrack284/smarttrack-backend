...
export class TeamService {
    ...
    async inviteMember(
        ...
        try {
            ...
            const savedInvite = await this.withTransaction(
                undefined,
                async trx => {
                    ...
                    await this.usageService.incrementTeamMemberCount(
                        companyId,
                        trx
                    );
                ...
                rule(
                    ...
                    () =>
                        new InternalErrorException(
                            "Unable to invite team member due to a data error. Please try again."
                        )
                ),
                rule(
                    Error,
    ...
    async listTeamMembersForCompany(
        ...
        try {
            ...
            // Fetch Supabase user details in parallel, with a fallback catch to prevent full failure if one user lookup fails
            const supabaseUsersPromises = members.map(member =>
                this.usersService
                    .getUserFromSupabase(member.userId as string)
                    .catch(() => null)
            );
            const supabaseUsers = await Promise.all(supabaseUsersPromises);

            // Merge database record fields with the Supabase avatarUrl
            const enrichedMembers = members.map((member, index) => {
                const supabaseUser = supabaseUsers[index];
                const avatarUrl =
            ...
            this.errorHandler.handle(
                err,
                "TeamService.listTeamMembersForCompany",
                [
                    rule(
                        QueryFailedError,
                        () =>
                            new InternalErrorException(
                                "Unable to list team members at this time. Please try again."
                            )
                    ),
                    rule(
                        Error,
    ...
    async cancelInvite(
        ...
        try {
            ...
            this.errorHandler.handle(err, "TeamService.cancelInvite", [
                rule(
                    QueryFailedError,
                    () =>
                        new InternalErrorException(
                            "Unable to cancel invite due to a data error. Please try again."
                        )
                ),
                rule(
                    Error,
    ...
    async resendInvite(
        ...
        try {
            ...
            this.errorHandler.handle(err, "TeamService.resendInvite", [
                rule(
                    QueryFailedError,
                    () =>
                        new InternalErrorException(
                            "Unable to resend invite due to a data error. Please try again."
                        )
                ),
                rule(
                    Error,
    ...
    async changeMemberRole(
        ...
        try {
            ...
            this.errorHandler.handle(err, "TeamService.changeMemberRole", [
                rule(
                    QueryFailedError,
                    () =>
                        new InternalErrorException(
                            "Unable to change role due to a data error. Please try again."
                        )
                ),
                rule(
                    Error,
    ...
    async removeMember(
        ...
        try {
            ...
            if (
                [TeamMemberStatus.ACTIVE, TeamMemberStatus.SUSPENDED].includes(
                    removedMember.status
                ) &&
                removedMember.userId
            ) {
                try {
                    // Fetch avatar filename from Supabase metadata
                    const supabaseUser =
                        await this.supabaseAdmin.auth.admin.getUserById(
                            removedMember.userId
                        );
                    const metadata = supabaseUser.data.user
                        ?.user_metadata as Record<string, unknown> | null;

                    const avatarFilename = metadata?.avatar_filename as
                        | string
                        | undefined;
                    ...
                    // Delete Supabase user directly (best effort)
                    ...
                    if (deleteError) {
                        this.logger.error({
                            msg: `Failed to delete Supabase user ${removedMember.userId} after team removal`,
                    ...
                    this.logger.error({
                        msg: `Post-removal cleanup failed for user ${removedMember.userId}.`,
                        err: (error as Error).message,
                        stack: (error as Error).stack
                    });
                    // Do NOT throw — the member is already removed from DB.
                }
            }
        } catch (err) {
            this.errorHandler.handle(err, "TeamService.removeMember", [
                rule(
                    QueryFailedError,
                    () =>
                        new InternalErrorException(
                            "Unable to remove team member due to a data error. Please try again."
                        )
                ),
                rule(
                    Error,
        ...
        try {
            ...
            if (suspendedMember.userId) {
                try {
                    ...
                    await this.usersService.banSupabaseUser(
                    ...
                    this.logger.error({
                        msg: `Failed to process post-suspension actions for user ${suspendedMember.userId}.`,
                        err: (error as Error).message,
                        stack: (error as Error).stack
                    });
            ...
            this.errorHandler.handle(err, "TeamService.suspendMember", [
                rule(
                    QueryFailedError,
                    () =>
                        new InternalErrorException(
                            "Unable to suspend member due to a data error. Please try again."
                        )
                ),
                rule(
                    Error,
    ...
    async getInviteByToken(token: string): Promise<{
        ...
        try {
            ...
            this.errorHandler.handle(err, "TeamService.getInviteByToken", [
                rule(
                    QueryFailedError,
                    () =>
                        new InternalErrorException(
    ...
    async acceptInvite(dto: AcceptInviteDto): Promise<{
        ...
        try {
            ...
            const { data, error } = await this.supabase.auth.admin.createUser({
                ...
                email_confirm: true, // invite flow = pre-verified
                user_metadata: {
            ...
            if (error || !data?.user) {
                ...
                // 400 = validation failure (weak password, bad email, etc.)
                if (
                    status === 400 ||
                    ...
                    throw new BadRequestAppException(
                        error?.message ||
                            "Account information does not meet requirements. Please check your details and try again."
                    );
                }

                this.logger.error({
                    msg: "Supabase admin.createUser failed during invite acceptance",
                    email: invite.email,
                    status,
                    err: error?.message
                });
            ...
            // Persist invite acceptance
            invite.userId = data.user.id;
            invite.name = dto.fullName.trim();
            ...
            // Emit event
            const payload =
                await this.buildTeamMemberAcceptedEventPayload(invite);
            this.events.emit(
            ...
            this.errorHandler.handle(err, "TeamService.acceptInvite", [
                rule(
                    ...
                    () =>
                        new InternalErrorException(
                            "Unable to accept invite due to a data conflict. Please try again."
                        )
                ),
                rule(
                    Error,
    ...
    async listAvailableDriversForCompany(companyId: string) {
        try {
            ...
            const filteredAvailableDrivers = drivers.filter(
                d =>
                    d.userId &&
                    !busySet.has(d.userId) &&
                    onlineDriverIds.has(d.userId)
            );
            ...
            for (const driver of filteredAvailableDrivers) {
                ...
                const supabaseDriver =
                    await this.usersService.getUserFromSupabase(driver.userId);

                availableDrivers.push({
                    ...driver,
                    avatarUrl:
                        (
                            supabaseDriver.user_metadata as Record<
                                string,
                                unknown
                            >
                        )?.avatar_url ?? null
            ...
            this.errorHandler.handle(
                err,
                "TeamService.listAvailableDriversForCompany",
                [
                    rule(
                        QueryFailedError,
                        () =>
                            new InternalErrorException(
                                "Unable to list available drivers at this time. Please try again."
                            )
                    ),
                    rule(
                        Error,
        ...
        try {
            ...
            if (activatedMember.userId) {
                try {
                    ...
                    await this.usersService.unbanSupabaseUser(
                    ...
                    this.logger.error({
                        msg: `Failed to process post-activation actions for user ${activatedMember.userId}.`,
                        err: (error as Error).message,
                        stack: (error as Error).stack
                    });
            ...
            this.errorHandler.handle(err, "TeamService.activateMember", [
                rule(
                    QueryFailedError,
                    () =>
                        new InternalErrorException(
                            "Unable to activate member due to a data error. Please try again."
                        )
                ),
                rule(
                    Error,
