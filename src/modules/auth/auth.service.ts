...
import {BadRequestAppException, InternalErrorException, UnauthorizedAppException,} from '#/common/exceptions';
...
export class AuthService {
  ...
  async resetPassword(email: string, token: string, newPassword: string) {
    try {
      ...
      const { error: updateError } =
        await this.supabaseAdmin.auth.admin.updateUserById(verifyData.user.id, {
          password: newPassword,
        });
      if (updateError) {
        throw new BadRequestAppException(
