...
export class AuthController {
  ...
  async googleCallback(
    ...
    try {
      const { session } = await this.authService.exchangeCodeForSession(
        code,
        request,
        reply,
      );
      // The SSR client already set the cookies via the `setAll` callback,
      // so we don't need to call this.setCookies again — they are already set.
