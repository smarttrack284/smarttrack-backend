...
export class OrdersService {
  ...
  async createOrder(
    ...
    return this.withTransaction(manager, async (trx) => {
      ...
      return { id: saved.id, orderReference: orderReference };
    });
  }
  ...
  async getOrderByReferenceForCompany(
    ...
    return { ...order, driverName: driver?.name };
  }
  ...
  async listOrdersForCompany(companyId: string, query: ListOrdersQueryDto) {
    ...
    return { orders: ordersWithDriverNames, total, page, pageSize };
  }
  ...
  async assignDriverForCompany(
    ...
    return this.withTransaction(manager, async (trx) => {
      const order = await trx.getRepository(Order).findOne({ where: { id } });
  ...
  async updateOrderForCompany(
    ...
    return this.withTransaction(manager, async (trx) => {
      ...
      const rejectedFields = this.getDisallowedFieldsInDto(dto, editableFields);
  ...
  async deleteOrderForCompany(
    ...
    await this.withTransaction(manager, async (trx) => {
      ...
      this.events.emit(ORDER_EVENTS.DELETED, new OrderDeletedEvent(companyId));
    });
  }
  ...
  async getKpiCountsForCompany(companyId: string, startOfToday: Date) {
    const raw = await this.orderRepo
      ...
      .where('order.companyId = :companyId', { companyId, startOfToday })
      .getRawOne();
  ...
  /**
   * Cache-aside wrapper around listOrdersForCompany — the underlying query
   * is unchanged (still the same QueryBuilder built earlier), just fronted
   * by a short-TTL cache keyed on the exact filter combination, so repeated
   * identical requests (e.g. two dispatchers viewing the same default filter)
  ...
  /**
   * Batch-resolves assignedDriverUserId -> driver name for a page of orders
   * in ONE query, not N+1. assignedDriverUserId is a bare Supabase user ID
   * with no FK relation to UserRole at the entity level (auth.users lives
   * in Supabase's own schema, not this database — same reasoning applied
   * everywhere else driver identity comes up, e.g.
   * AnalyticsService.computeTopDrivers). companyId is included in the WHERE
   * clause as defense-in-depth, not just an optimization — it means this
   * can never accidentally resolve a name from a DIFFERENT company's
   * UserRole rows even if a driver ID were somehow malformed or reused.
   */
  private async getDriverNamesForOrders(
    ...
    const drivers = await this.userRoleRepo.find({
      where: { companyId, userId: In(driverIds) },
