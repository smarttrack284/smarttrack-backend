...
import {EditableOrderField, getEditableFieldsForStatus,} from '#/common/constants/order-editable-fields.constant';

const LOCKED_ORDER_STATUSES = new Set<OrderStatus>([
  ...
  async updateOrderForCompany(
    ...
    return this.withTransaction(manager, async (trx) => {
      ...
      if (rejectedFields.length > 0) {
        throw new BadRequestAppException(
          `These fields can't be changed while the order is ${order.status}: ${rejectedFields.join(', ')}`,
