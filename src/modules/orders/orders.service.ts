...
import { OrderApiResponseDto } from "#/modules/orders/dto/order-api-response.dto";
import {
...
export class OrdersService {
    ...
    async createOrder(
        ...
        try {
            ...
            this.errorHandler.handle(err, "OrdersService.createOrder", [
                // Unique constraint violation (tracking number or order reference)
                rule(QueryFailedError, e => {
                    const pg = (e as any).driverError;
                    if (pg?.code === "23505") {
                        return new ResourceConflictException(
                            "Order creation failed due to a conflict. Please try again."
                        );
                    }
                    // Any other query error (connection, FK, etc.)
                    return new InternalErrorException(
                        "Unable to create order at this time. Please try again."
                    );
                }),
                // Catch‑all for truly unexpected runtime errors
                rule(
                    Error,
                    () =>
    ...
    async importOrdersFromCsv(
        ...
        if (fileBuffer.length > MAX_IMPORT_FILE_SIZE_BYTES) {
            throw new BadRequestAppException(
                "CSV file exceeds the 2MB size limit"
            );
        }
        ...
        try {
            ...
            // Original error is logged server‑side by the error handler if
            // you choose to wrap this method later; for now, give a safe message.
            throw new BadRequestAppException(
                "Could not parse CSV file. Please check that the file is a valid CSV."
            );
        }
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            const createDto = mapCsvRowToCreateOrderDto(
                rowDto,
                parsedItems.items
            );

            try {
                await this.createOrder(createDto, createdByUserId, companyId, false);
                imported++;
            } catch (err) {
                // Plan limit reached – stop importing further rows.
                ...
                // Map the error to a safe, client‑facing message.
                const safeMessage = this.toSafeError(err);
                failed.push({ row: rowNumber, errors: [safeMessage] });
            }
        }
     ...
     */
    async createOrderForCompanyViaApi(
        companyId: string,
        dto: CreateOrderDto,
        manager?: EntityManager
    ): Promise<OrderApiResponseDto> {
        try {
            return this.withTransaction(manager, async trx => {
                await this.usageService.incrementOrderCount(companyId, trx);
                const trackingNumber =
                ...
                this.events.emit(
                    ORDER_EVENTS.CREATED,
                    new OrderCreatedEvent({ ...payload, status: saved.status })
                );
                ...
                return {
                    ...
                    scheduledFor:
                        saved.scheduledFor?.toISOString() ?? undefined,
            ...
            this.errorHandler.handle(
                err,
                "OrdersService.createOrderForCompany",
                [
                    rule(QueryFailedError, e => {
                        const pg = (e as any).driverError;
                        if (pg?.code === "23505") {
    ...
    async getOrderByTrackingNumberForCompany(
        ...
        try {
            ...
            // This check is technically redundant (the query already filters by companyId),
            // but if it ever fails, it means a data integrity issue – still safe to expose.
            ...
            return {
                ...
                scheduledFor: order.scheduledFor?.toISOString() ?? undefined,
                createdAt: order.createdAt.toISOString()
            };
        } catch (err) {
            this.errorHandler.handle(
                err,
                "OrdersService.getOrderByTrackingNumberForCompany",
                [
                    rule(
                        QueryFailedError,
                        () =>
                            new InternalErrorException(
                                "Unable to retrieve order details at this time. Please try again."
                            )
                    ),
                    rule(
                        Error,
    ...
    async listOrdersForCompany(companyId: string, query: ListOrdersQueryDto) {
        try {
            ...
            this.errorHandler.handle(
                err,
                "OrdersService.listOrdersForCompany",
                [
                    // Database query failure – e.g. connection lost, timeout.
                    rule(
                        QueryFailedError,
                        () =>
                            new InternalErrorException(
                                "Unable to retrieve orders at this time. Please try again."
                            )
                    ),
                    // Any other unexpected runtime error.
                    rule(
    ...
    async listOrdersForCompanyViaApi(
        companyId: string,
        query: ListOrdersQueryDto
    ): Promise<{
        orders: OrderApiResponseDto[];
        total: number;
        page: number;
        pageSize: number;
    }> {
        try {
            ...
            // Map each entity to the safe API DTO – never expose raw entity.
            const ordersDto: OrderApiResponseDto[] = orders.map(order => ({
                id: order.id,
                trackingNumber: order.trackingNumber,
                orderReference: order.orderReference,
                status: order.status,
            ...
            return { orders: ordersDto, total, page, pageSize };
        } catch (err) {
            this.errorHandler.handle(
                err,
                "OrdersService.listOrdersForCompanyViaApi",
                [
                    rule(
                        QueryFailedError,
                        () =>
            ...
            this.errorHandler.handle(
                ...
                [
                    rule(
                        ...
                        () =>
                            new InternalErrorException(
                                "Unable to update order status at this time. Please try again."
                            )
                    ),
                    rule(
                        Error,
    ...
    async assignDriverForCompany(
        ...
        try {
            ...
            this.errorHandler.handle(
                ...
                [
                    // Database error during assignment (save failure, connection, etc.)
                    rule(
                        QueryFailedError,
                        () =>
                            new InternalErrorException(
                                "Unable to assign driver at this time. Please try again."
                            )
                    ),
                    // Any other unexpected runtime error
                    rule(
            ...
            this.errorHandler.handle(
                ...
                [
                    rule(
                        ...
                        () =>
                            new InternalErrorException(
                                "Unable to update order at this time. Please try again."
                            )
                    ),
                    rule(
                        Error,
        ...
        try {
            await this.withTransaction(manager, async trx => {
                ...
                const deletedBy = await trx.getRepository(UserRole).findOne({
                    where: { companyId, userId: deletedByUserId }
                });
            ...
            this.errorHandler.handle(
                ...
                [
                    rule(
                        ...
                        () =>
                            new InternalErrorException(
                                "Unable to delete order at this time. Please try again."
                            )
                    ),
                    rule(
                        Error,
            ...
            this.errorHandler.handle(
                ...
                [
                    rule(
                        ...
                        () =>
                            new InternalErrorException(
                                "Unable to retrieve performance metrics at this time. Please try again."
                            )
                    ),
                    rule(
                        Error,
    ...
    async listOrdersForCompanyCached(
        ...
        try {
            ...
            this.errorHandler.handle(
                err,
                "OrdersService.listOrdersForCompanyCached",
                [
                    rule(
                        QueryFailedError,
                        () =>
    ...
    private async buildOrderEventPayload(
        ...
        return {
            ...
            orderUrl: `${this.config.get("CLIENT_URL")}/dashboard/orders/${
                order.id
            }`,
    ...
    /**
     * Extracts a production‑safe error message suitable for the failed‑rows
     * report. AppExceptions carry messages that are already safe for the
     * client; everything else is replaced with a generic failure string so
    ...
    private toSafeError(error: unknown): string {
        ...
        // Log the real error here if desired, but never expose it.
        return "An unexpected error occurred while processing this row.";
    }
}
