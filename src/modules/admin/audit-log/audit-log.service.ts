...
export class AdminAuditLogService {
  ...
  async listAuditLogs(dto: ListAdminAuditLogsDto) {
    ...
    try {
      ...
      this.errorHandler.handle(err, 'AdminAuditLogService.listAuditLogs', [
        rule(
          QueryFailedError,
          () =>
  ...
  private async queryAuditLogs(dto: ListAdminAuditLogsDto) {
    ...
    if (dto.search) {
      qb.andWhere(
        new Brackets((sqb) => {
          sqb
            .where('log.message ILIKE :search', { search: `%${dto.search}%` })
            .orWhere('log.action ILIKE :search', { search: `%${dto.search}%` });
        }),
      );
    }
    ...
    const adminDetailsMap = new Map<string, { name: string; email: string }>();
      ...
      await Promise.all(
        adminUserIds.map(async (adminUserId) => {
          try {
            ...
            if (user) {
              adminDetailsMap.set(adminUserId, {
                name:
    ...
    if (dto.severity) {
      countQb.andWhere('log.severity = :severity', { severity: dto.severity });
    }
    if (dto.search) {
      countQb.andWhere(
        new Brackets((sqb) => {
