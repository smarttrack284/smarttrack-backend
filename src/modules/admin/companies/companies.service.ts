...
import {getPlanUsageLimits, SubscriptionPlan, SubscriptionStatus,} from '#/common/constants/subscription-plan.constant';
...
import {ListCompanyOrdersDto, OrderSort,} from '#/modules/admin/companies/dto/list-company-orders.dto';
...
export class AdminCompaniesService {
  ...
  async listCompanies(dto: ListCompaniesDto) {
    ...
    try {
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.listCompanies', [
        rule(
          QueryFailedError,
          () =>
  ...
  async getCompanyDetail(companyId: string, dto: GetCompanyDetailDto) {
    ...
    try {
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.getCompanyDetail', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
  ...
  async changeCompanyPlan(
    ...
    try {
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.changeCompanyPlan', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
  ...
  async sendPasswordReset(
    ...
    try {
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.sendPasswordReset', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
  ...
  async listCompanyOrders(companyId: string, dto: ListCompanyOrdersDto) {
    const cacheKey = `admin:companies:${companyId}:orders:${this.buildOrdersCacheKey(dto)}`;
    ...
    try {
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.listCompanyOrders', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
  ...
  async revokeApiKey(
    ...
    try {
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.revokeApiKey', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
  ...
  async suspendCompany(companyId: string, adminUserId: string) {
    try {
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.suspendCompany', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
  ...
  async reactivateCompany(companyId: string, adminUserId: string) {
    try {
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.reactivateCompany', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
  ...
  async listWebhookDeliveries(
    ...
    const cacheKey = `admin:companies:${companyId}:webhook-deliveries:${this.buildWebhookDeliveriesCacheKey(dto)}`;
  ...
  async toggleWebhookEndpoint(
    ...
    try {
      ...
      if (!endpoint) {
        throw new ResourceNotFoundException('Webhook endpoint not found');
      }
      ...
      await this.withTransaction(undefined, async (trx) => {
        ...
        await webhookRepo.update({ id: endpointId }, { isActive: newIsActive });
      });
      ...
      await this.activityLogService.record({
        ...
        severity: ActivitySeverity.WARNING,
        message: `Admin ${newIsActive ? 'enabled' : 'disabled'} webhook endpoint ${endpoint.description || endpoint.id}`,
        actorUserId: null,
        actorName: null,
  ...
  async deleteCompany(companyId: string, adminUserId: string) {
    try {
      ...
      await this.withTransaction(undefined, async (trx) => {
        ...
        await webhookDeliveryRepo.delete({ webhookEndpoint: { companyId } }); // or using query
      ...
      this.errorHandler.handle(err, 'AdminCompaniesService.deleteCompany', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
  ...
  private async queryWebhookDeliveries(
    ...
    if (dto.status) {
      countQb.andWhere('delivery.status = :status', { status: dto.status });
    }
  ...
  private async queryCompanyOrders(
    ...
    if (dto.search) {
      qb.andWhere(
        new Brackets((sqb) => {
          sqb
            .where('order.trackingNumber ILIKE :search', {
              search: `%${dto.search}%`,
            })
    ...
    if (dto.dateTo) {
      const endOfDay = new Date(dto.dateTo);
      endOfDay.setHours(23, 59, 59, 999);
      countQb.andWhere('order.createdAt <= :dateTo', { dateTo: endOfDay });
    }
  ...
  private async buildCompanyDetail(
    ...
    const [subscription, usage, owner, teamMembersResult, apiKeys, webhooks] =
      await Promise.all([
