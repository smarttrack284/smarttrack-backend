...
export class DispatchService {
  ...
  async dispatchOrdersToDriver(
    ...
    try {
      ...
      const savedTrip = await this.withTransaction(undefined, async (trx) => {
        const tripReference = await this.generateUniqueTripReference(trx);
        ...
        for (let i = 0; i < dto.orderIds.length; i++) {
          ...
          // Prevent manual dispatch of scheduled orders
          if (order.scheduledFor) {
            throw new BadRequestAppException(
              'One or more selected orders have a scheduled delivery time and cannot be dispatched manually.',
            );
          }
          ...
          await this.ordersService.assignDriverForCompany(
            orderId,
            companyId,
            dto.driverUserId, // driver (assigned to)
            dispatcherUserId, // dispatcher (who assigned)
            trx,
          );
        ...
        rule(
          ...
          () =>
            new InternalErrorException(
              'Unable to dispatch orders at this time. Please try again.',
            ),
        ),
        rule(
          Error,
    ...
    try {
      ...
      this.errorHandler.handle(err, 'DispatchService.getTripForCompany', [
        rule(
          ...
          () =>
            new InternalErrorException(
              'Unable to retrieve trip details at this time. Please try again.',
            ),
        ),
        rule(
          Error,
  ...
  async listTripsForCompany(companyId: string, query: ListTripsQueryDto) {
    try {
      ...
      this.errorHandler.handle(err, 'DispatchService.listTripsForCompany', [
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
              'Unable to retrieve trips at this time. Please try again.',
            ),
        ),
        rule(
          Error,
    ...
    try {
      ...
      this.errorHandler.handle(err, 'DispatchService.arriveAtStop', [
        rule(
          ...
          () =>
            new InternalErrorException(
              'Unable to update stop status at this time. Please try again.',
            ),
        ),
        rule(
          Error,
  ...
  async completeStop(
    ...
    // Track uploaded files so we can clean them up if the transaction fails
    ...
    try {
      ...
      if (files.photo) {
        const path = StoragePath.proofOfDelivery(
          companyId,
          stopId,
      ...
      if (uploadedPhotoPath) {
        await this.storageService
          ...
          .catch((cleanupErr) =>
            this.logger.error(
              `Failed cleaning PoD photo: ${uploadedPhotoPath}`,
              cleanupErr,
            ),
          );
      }
      if (uploadedSignaturePath) {
        await this.storageService
          ...
          .catch((cleanupErr) =>
            this.logger.error(
              `Failed cleaning PoD signature: ${uploadedSignaturePath}`,
              cleanupErr,
            ),
          );
      }
      ...
      this.errorHandler.handle(err, 'DispatchService.completeStop', [
        // Storage / file processing errors
        rule(Error, (e) => {
          if (
        ...
        // Database errors during the transaction
        rule(
          QueryFailedError,
          () =>
            new InternalErrorException(
              'Unable to complete delivery at this time. Please try again.',
            ),
        ),
    ...
    try {
      ...
      this.errorHandler.handle(err, 'DispatchService.skipStop', [
        rule(
          ...
          () =>
            new InternalErrorException(
              'Unable to skip stop at this time. Please try again.',
            ),
        ),
        rule(
          Error,
  ...
  async listTripsForCompanyWithDriverNames(
    ...
    try {
      ...
      this.errorHandler.handle(
        err,
        'DispatchService.listTripsForCompanyWithDriverNames',
        [
          rule(
            QueryFailedError,
            () =>
  ...
  private async generateUniqueTripReference(
    manager: EntityManager,
    attempt = 0,
  ): Promise<string> {
    if (attempt >= 5) {
      throw new ResourceConflictException(
        'Could not generate a unique trip reference — please retry',
      );
    }
    const candidate = generateTripReference();
    const existing = await manager
      .getRepository(Trip)
      .findOne({ where: { tripReference: candidate } });
    return existing
      ? this.generateUniqueTripReference(manager, attempt + 1)
      : candidate;
  }
}
