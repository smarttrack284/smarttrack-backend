...
export class DispatchService {
    ...
    async dispatchOrdersToDriver(
        ...
        try {
            ...
            const savedTrip = await this.withTransaction(
                ...
                async trx => {
                    const tripReference =
                        await this.generateUniqueTripReference(trx);
    ...
    async listTripsForCompany(companyId: string, query: ListTripsQueryDto) {
        try {
            ...
            this.errorHandler.handle(
                err,
                "DispatchService.listTripsForCompany",
                [
                    rule(
                        QueryFailedError,
                        () =>
    ...
    async listTripsForCompanyWithDriverNames(
        ...
        try {
            ...
            const drivers = await this.getDriversForTrips(
                companyId,
                result.trips
            );
    ...
    private async getStopForCompany(
        ...
        if (!trip)
            throw new ResourceNotFoundException("This trip cannot be found");
        if (trip.companyId !== companyId) {
            throw new ForbiddenAppException(
        ...
        if (!stop)
            throw new ResourceNotFoundException(
                "This trip stop cannot be found"
            );
