    ...
    const savedTrip = await this.withTransaction(undefined, async (trx) => {
      ...
      for (let i = 0; i < dto.orderIds.length; i++) {
        ...
        if (order.status !== OrderStatus.PENDING) {
          // BUG FIX: order.orderReference doesn't exist on the
          // Order entity — this threw at runtime the moment a
          // non-pending order was dispatched. trackingNumber is
          // the entity's actual customer-facing identifier.
          throw new BadRequestAppException(
            `Order #${order.trackingNumber} is no longer pending and can't be dispatched`,
          );
        }
      ...
      trip_.stops = stops;
      // BUG FIX: the method built `stops` but never returned the
      // trip — dispatchOrdersToDriver resolved to `undefined`,
      // which the controller then tried to look up by `.id` and
      // would have thrown a confusing TypeError, not a clean 4xx.
      return trip_;
    });
  ...
  async arriveAtStop(
    ...
    const savedStop = await this.withTransaction(undefined, async (trx) => {
      ...
      if (stop.status !== StopStatus.PENDING) {
        throw new BadRequestAppException(`This stop is already ${stop.status}`);
      }
