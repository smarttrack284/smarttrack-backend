...
export class DispatchService {
    /** The order statuses a stop action walks through, in sequence — a stop action never jumps straight to a target status, it hops one valid transition at a time through OrdersService, so ALLOWED_ORDER_STATUS_TRANSITIONS stays the single source of truth. */
    ...
    async dispatchOrdersToDriver(
        ...
        return this.withTransaction(undefined, async trx => {
            ...
            const savedTrip = await trx.getRepository(Trip).save(trip);
    ...
    async listTripsForCompany(companyId: string, query: ListTripsQueryDto) {
        ...
        if (query.search) {
            ...
            filtered = filtered.filter(trip =>
                trip.stops.some(
                    stop =>
                        stop.order.orderReference.toLowerCase().includes(q) ||
     ...
     * through ASSIGNED -> PICKED_UP -> IN_TRANSIT via OrdersService (whatever
     * hops remain from its current status), then marks the stop ARRIVED.
     */
    async arriveAtStop(
        tripId: string,
        stopId: string,
    ...
    /** Driver has delivered this stop's order. Order goes IN_TRANSIT -> DELIVERED. */
    async completeStop(
        tripId: string,
        stopId: string,
        companyId: string
    ...
    /**
     * Driver couldn't complete this stop. The order becomes FAILED (see
     * order-status.ts's existing "Retry delivery" action for FAILED orders —
     * this closes directly into that, rather than inventing a new order
     * state) and the trip continues to its next stop; the driver isn't
    ...
    /** Walks an order forward through ORDER_PROGRESSION, one valid hop at a time, until it reaches `target` — never skips a transition OrdersService wouldn't otherwise allow. No-op if the order is already at or past the target on the known progression. */
