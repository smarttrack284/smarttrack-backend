...
export class TeamNotificationsService {
  ...
  async handleTeamMemberAccepted(event: TeamMemberAcceptedEvent): Promise<void> {
    try {
      if (!event.payload.recipients.length) return;

      const companySettings = await this.getCompanySettings(event.payload.companyId);
  ...
  async handleTeamInviteMember(event: TeamInviteMemberEvent): Promise<void> {
    try {
      ...
      // Invites generally bypass the master switch (needs no company settings check)
      ...
      this.logger.error(
        `handleTeamInviteMember failed to send invite to ${event.payload.inviteEmail}`,
        (err as Error).stack,
      );
    }
  }

  async handleTeamMemberSuspended(event: TeamMemberSuspendedEvent): Promise<void> {
    try {
  ...
  async handleTeamMemberActivated(event: TeamMemberActivatedEvent): Promise<void> {
    try {
  ...
  private async sendTeamMemberAcceptedEmail(
    ...
    try {
      ...
      this.logger.error(
        `Failed to send member accepted email to ${email}`,
        (err as Error).stack,
      );
    }
  }
  ...
  private async sendTeamEmailNotifications(
    ...
    try {
      ...
      const emails = event.payload.teamRecipients.map((recipient) =>
        this.sendEmail(recipient.email, recipient.name as string, event, status),
      );
      await Promise.all(emails);
    } catch (err) {
      this.logger.error(
        `sendTeamEmailNotifications failed for company ${event.payload.companyId}`,
        (err as Error).stack,
      );
    }
  }
  ...
  private async sendEmail(
    ...
    try {
      ...
      this.logger.error(
        `Failed to send team notification email to ${email} for order ${event.payload.orderReference}`,
        (err as Error).stack,
      );
    }
  }

  private async getCompanySettings(companyId: string): Promise<CompanyNotificationSetting | null> {
    try {
      ...
      this.logger.error(
        `Failed to fetch company notification settings for ${companyId}`,
        (err as Error).stack,
      );
