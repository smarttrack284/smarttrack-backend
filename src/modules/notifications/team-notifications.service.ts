...
export class TeamNotificationsService {
    ...
    async handleTeamInviteMember(event: TeamInviteMemberEvent): Promise<void> {
        try {
            ...
            this.logger.error({
                msg: `handleTeamInviteMember failed to send invite to ${maskEmail(event.payload.inviteEmail)}`,
                err: (err as Error).message,
                stack: (err as Error).stack,
            });
        }
    ...
    private async sendTeamMemberAcceptedEmail(
        ...
        try {
            ...
            this.logger.error({
                msg: `Failed to send member accepted email to ${maskEmail(email)}`,
                err: (err as Error).message,
                stack: (err as Error).stack,
            });
        }
    ...
    private async sendEmail(
        ...
        try {
            ...
            this.logger.error({
                msg: `Failed to send team notification email to ${maskEmail(email)} for order ${event.payload.orderReference}`,
                err: (err as Error).message,
                stack: (err as Error).stack,
            });
        }
